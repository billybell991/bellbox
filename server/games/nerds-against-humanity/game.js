...
export class GameRoom {
  ...
  startGame(selectedThemes, spiceLevel = 3) {
    if (this.playerOrder.length < 3) return { error: 'Need at least 3 players' };

    if (selectedThemes && selectedThemes.length > 0) {
      this.selectedThemes = selectedThemes;
    }
    const deck = buildDeck(this.selectedThemes, spiceLevel);

    if (deck.blackCards.length < 10 || deck.whiteCards.length < 30) {
      return { error: 'Not enough cards at this spice level. Try adding more card packs or raising the spice!' };
    }
