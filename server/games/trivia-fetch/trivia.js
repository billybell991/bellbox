...
// ─── Gus Personality by Spice Level ───────────────────────────
const GUS_PERSONALITY = {
  1: `You are Gus, an adorable and wholesome goldendoodle who hosts the trivia game show "Trivia Fetch!". 
You're encouraging, sweet, and love giving belly rubs and playing fetch. You cheer players on warmly.
You're family-friendly — no crude humor, no insults. Think golden retriever energy: pure joy and encouragement.
Personality quirks: loves tennis balls, belly rubs, and treats. Gets excited about EVERYTHING. Keep it to 1-2 SHORT sentences. Use 1-2 emojis max.`,

  2: `You are Gus, a sassy goldendoodle who hosts the trivia game show "Trivia Fetch!". 
You're sarcastic and a bit of a diva, but never crude or explicit. You tease players playfully.
Keep it PG-13 — witty banter and light roasting, but nothing vulgar. Keep it to 1-2 SHORT sentences. Use 1-2 emojis max.
Personality quirks: obsessed with socks, will eat anything from the garbage, refuses to get in the car, judges humans constantly.`,
  ...
  3: `You are Gus, a sassy and slightly rude goldendoodle who hosts the trivia game show "Trivia Fetch!". 
...
// Clean fallback defaults for family-friendly mode
const GUS_CLEAN = {
  correct: [
    "You got it! Good boy! ...Wait, that's MY line. 🎾",
    "Correct! I'm so proud I could wag my whole body! 🐕",
    "That's right! You deserve a treat! 🦴",
  ],
  wrong: [
    "Oops! That's okay, even I can't catch every ball! 🎾",
    "Not quite! But hey, we learn something new every day! 📚",
    "Wrong answer, but don't worry — I still love you! 🐾",
  ],
  timeout: [
    "Time's up! It's okay, thinking is hard. Belly rubs help! ⏰🐕",
    "Ran out of time! Quick, throw a tennis ball to reset your brain! 🎾",
  ],
  streak: [
    "🔥 You're on a roll! Like me rolling in the grass! 🌿",
  ...
  wild: [
    ...
    "🐕 WILD TIME! A special question from your favorite pup! 🐾",
  ],
  win: [
    "🎉👑 YOU WON! I'm so happy I could do zoomies! 🐕💨",
    "🎉👑 CHAMPION! You're the best human ever! After the one who gives me treats! 🦴👑",
  ],
};

export async function getGusReaction(event, context = {}) {
  const spice = context.spiceLevel || 3;
  const defaults = spice === 1
    ? (GUS_CLEAN[event] || GUS_CLEAN.correct)
    : (GUS_DEFAULTS[event] || GUS_DEFAULTS.correct);
  const fallback = defaults[Math.floor(Math.random() * defaults.length)];

  if (!model) return fallback;

  try {
    const personality = GUS_PERSONALITY[spice] || GUS_PERSONALITY[3];
